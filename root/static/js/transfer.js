...
var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        // 总个数显示文本
        var total_num_str = settings.data.length + " 项目";

        // 分组总个数
        var total_group_num = getGroupNum(groupData, groupListName);
        // 分组总个数显示文本
        var total_group_num_str = total_group_num + " 项目";
        ...
        $(transferId).find(tabItemName).on("click", function () {
            ...
            if (!$(this).is(".tab-active")) {
                ...
                // 清空右侧数量
                $(transferId).find(selectedTotalNum).text("共0 项目");
                // 未选中项
                if ($(transferId).find(tabContentFirst).css("display") != "none") {
                    ...
                    $(transferId).find(groupTotalNum).empty();
                    $(transferId).find(groupTotalNum).append($(transferId).find(transferDoubleGroupListLiUlLi).length + " 项目");
                } else {
                    // 分组
                    ...
                    $(transferId).find(totalNum).empty();
                    $(transferId).find(totalNum).append($(transferId).find(transferDoubleListLi).length + " 项目");
                }
                // 数据变化触发回调
        ...
        $(addSelected).on("click", function () {
            ...
            if ($(transferId).find(tabContentFirst).css("display") != "none") {
                ...
                var new_total_num_str = new_group_total_num + " 项目";
                // 左侧数量
                $(transferId).find(groupTotalNum).append(new_total_num_str);
                // 右侧数量
                $(transferId).find(selectedTotalNum).text(selected_total_num + " 项目");
                ...
                // 计算右侧总数
                selected_total_num = selectedItemNum;
                var new_total_num_str = new_total_num + " 项目";
                // 左侧数量
    ...
    function generateTransfer(inputId, currentTimeStr) {
        var htmlStr =
            ...
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + ' tab-active">菜单</div>'
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + '">路径</div>'
            + '</div>'
            ...
            + '</div>'
            + '<div class="transfer-double-list-footer">'
            + '<label class="selected_total_num_' + currentTimeStr + '">0 项目</label>'
            + '</div>'
            + '</div>'
            + '</div>'
            + '</div>'
