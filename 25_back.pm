...
sub auto : Private {
    ...
    my ( $self, $c ) = @_;
    # 获取用户名
    my $username = $c->req->cookies->{ sessionid }->{value}->[0] || "";
    ...
    # 登录控制
    if ( $c->{ request }->{ action } =~ m/^$c->{ base_url }\/login$/ ){   # 正则匹配url
        $c->detach( "login" );                                        # 访问的是登录页面直接跳转，不执行获取session url 不重定向
    }
    elsif ( $c->{ request }->{ action } =~ m/^$c->{ base_url }\/logout$/ ) {
         $c->detach( "logout" );
    }
    else {      # 处理登录后的事情
        if ( $username eq "" ) {                                    # 没有用户名
            $c->response->redirect(	                                # url重定向跳转到登录页面
                $c->uri_for( $c->controller( '2020::10::25' )->action_for( 'login' ) )
            );                                                      # 访问的不是登录页面，没有登录的时候进入登录页面 url重定向
            $c->detach();
        }
        my $res = ToolFunc::get_session( $c->{ sessionid } );               # 获取session状态
        if ( $res eq 0 ) {                                              # 没有登录过
            $c->response->redirect(	                                # url重定向跳转到登录页面
                $c->uri_for( $c->controller( '2020::10::25' )->action_for( 'login' ) )
            );                                                      # 访问的不是登录页面，没有登录的时候进入登录页面 url重定向
        ...
        # 刷新redis寿命3600
        ToolFunc::set_session_cover_pre( $c->{ sessionid }, $res );
        # 非登录页面的权限控制
...
sub index :Path :Args(0) {
    ...
    if ( $res eq 0) {
        ...
        $c->{ msg } = "非法访问：512";      # 512 没有查询到用户信息 非法
        $c->{ status_code } = 403;
        $c->detach( "error");
        return 1;
	}
...
=head1 login
    ...
    登录逻辑需要写在当前模块中，因为子模块的访问需要经过auto，会进入跳转死循环，auto控制流需要判断访问路径是login页面还是其它页面，
    访问的是login页面则跳转到当前函数，如果访问的是其它页面，若session存在，则放行，若不存在则重定向到当前函数进行登录认证操作
    该业务逻辑主要是验证密码准确性，以及获取当前用户权限，将其放入session中
=cut

sub login : Local {
    my ( $self, $c ) = @_;
    # 如果是post数据 则进行登录验证， 返回json
    if ( ToolFunc::get_session( $c->{ sessionid } ) > 0 ) {       # 存在session
        $c->response->redirect(	                                # 登录成功 url重定向跳转到后台首页
                $c->uri_for( $c->controller( '2020::10::25' )->action_for( 'index' ) )
        );
        $c->detach();
    }
    if ( $c->{ request }->{ method } eq "POST" ) {              # GET方式返回页面
        my $username = $c->{ request }->{ parameters }->{ username };
        my $password = $c->{ request }->{ parameters }->{ password };
        my $sql = "select * from usr_wfw.T_FX_GLY where ZHMC=? and DLMM=?";     # 预处理sql
        my $rtn = DB::auth($sql, $username, $password );            # 用户名密码认证
        if ( $rtn eq '1' ) {
            # 获取用户权限写入session
            set_privilege($username, $c);
            $c->{ sessionid } = $username;      # 登录传入end进行session写入
            $c->response->redirect(	                                # 登录成功 url重定向跳转到后台首页
                $c->uri_for( $c->controller( '2020::10::25' )->action_for( 'index' ) )
            );
        }
        elsif ( $rtn eq '-1'){
            $c->{ msg } = "登录验证出错，已经记录日志：513";      # 512 可能的sql注入
            $c->{ status_code } = 500;
            $c->detach( "error");
            return 1;
        }
